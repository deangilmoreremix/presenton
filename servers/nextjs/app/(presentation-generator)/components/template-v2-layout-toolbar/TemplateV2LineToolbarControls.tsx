...
type LinePanelId = "line-width" | "line-color" | "line-style";
  ...
  return (
    <>
          ...
          <Panel className="w-[200px] overflow-hidden rounded-[12px] border border-[#E8E9EE] py-2 shadow-[0_8px_24px_rgba(16,24,40,0.12)]">
            {LINE_STYLE_OPTIONS.map((option) => (
              ...
              >
                ...
                {currentStyle.key === option.key ? <PatternIcon /> : null}
              </button>
            ))}
          </Panel>
        ) : null}
      ...
      <button
        ...
        onClick={() => {
          const currentIndex = LINE_STYLE_OPTIONS.findIndex(
            (option) => option.key === currentStyle.key,
          );
          const nextStyle =
            LINE_STYLE_OPTIONS[(currentIndex + 1) % LINE_STYLE_OPTIONS.length];
          setStroke({ dash: nextStyle.dash });
        }}
        className="flex items-center rounded-[4px] p-1 text-[#191919] hover:bg-[#F8F8FA]"
      >
        <PatternIcon />
      </button>
    </>
